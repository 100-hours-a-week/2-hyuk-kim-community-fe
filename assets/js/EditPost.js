...
import {GET_POST_EDIT_URL, GET_POST_HEADER, PATCH_POST_URL, PATCH_POST_HEADER} from "./api/constants.js";
...
async function fetchGetPostEdit() {
    apiFetch(GET_POST_EDIT_URL.replace(":postId", postId), GET_POST_HEADER).then((result) => {
        console.log("get post success!! : " + result);
        if(result) {
            document.querySelector(".input-title").value = result["title"];
            document.querySelector(".input-content").textContent = result["content"];
...
async function fetchUpdatePost(title, content) {
    ...
    apiFetch(PATCH_POST_URL.replace(":postId", postId), PATCH_POST_HEADER, body).then((result) => {
        if(result) {
